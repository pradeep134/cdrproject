...
# Declaring the local  and Global variables

project_name=None

app = dash.Dash(external_stylesheets=[dbc.themes.BOOTSTRAP])
                                                               
#Declaring the mutliple functions

def load_data():  
    
    print("starting of the load_data function")  
...
def open_browser():
    webbrowser.get('windows-default').open_new('http://127.0.0.1:8050/')
    
    
def create_app_ui():
     main_layout = html.Div([

     html.H1(html.Marquee(children='CDR ANALYSIS WITH INSIGHTS',
             id='Main_title',  
             style = {"align":"center","textAlign":"center","color":"blue",'background-color':'pink'})),
      ...
      dcc.Dropdown(
       ...
       style = {"background-color":"yellow",'Font-weight': 'bold'},children = [            
       html.Br(),
       dcc.Dropdown(
      ...
      html.Br()]),
      dcc.Tab(label = "Service Analytics tool", id="Service Analytics tool", value="tab-3",style = {"background-color":"yellow"},children = [            
      html.Br(),
      dcc.Dropdown(
...
def create_card(title, content, color):
    card = dbc.Card(
        dbc.CardBody(
            [ html.H4(title),  html.Br(),  html.Br(), html.H2(content), html.Br(),  ]
...
def update_app_ui(Tabs,start_date, end_date, group, report_type,device_date,service_date):
    ...
    if Tabs == "tab-1":
        ...
        card_2=create_card("Incoming Calls",incoming_calls, "dark")
        ...
        card_5=create_card("Maximim Duration", f'{max_duration} min', "dark")
      
        card_6 = create_card(title, content, "success")
...
def elupdate_groups(start_date, end_date ):
     print("data type",  str(type(start_date)))
     print("data value", str(start_date))
...
def main():
    ...
    project_name="call Data Analytics"
    
    global  call_data,service_data,device_data,start_date_list ,end_date_list,report_type                                                       # after completing the project just dealloctae the memory
